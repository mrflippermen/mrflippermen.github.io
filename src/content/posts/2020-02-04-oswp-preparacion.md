...
classes: wide
header:
  teaser: /images/oswp-preparation/oswp-logo.png
...
#### Â¿Qué bases tuve antes de comenzar con la certificación?
...
#### Â¿Qué pasos me recomiendas para abordar con éxito la certificación?
...
#### Â¿Cómo está estructurado el examen?
...
La gran pregunta, Â¿son cuatro horas suficientes?, mi respuesta es más que suficiente. En mi caso en una media
...
#### Â¿Cuáles son los siguientes pasos?
...
WPA y por otro lado en ataques a redes WEP con sus distintos casos, Â¡así que así lo haremos Mike!
...
por ejemplo, si no vamos a tener acceso a internet... Â¿por qué tener corriendo los procesos '**dhclient**' y
...
Importante, Â¿cómo desactivar el modo monitor y hacer que todo vuelva a la normalidad en términos de
...
AGENCY** (**NSA**), Â¿cómo se procedería?. Primero buscamos la dirección MAC en el amplio listado del que
dispone 'macchanger':
...
Una dirección MAC está compuesta por 6 bytes, ya tenemos los primeros 3 bytes, Â¿qué hay de los otros 3 bytes?.
...
Entonces bien, Â¿cómo se interpreta este output?. 
...
```bash
 ...
 Imaginemos que queremos filtrar para que sólo se lista el punto de acceso cuyo **ESSID** es **hacklab**, Â¿qué
...
Podrías pensar, Â¿entonces tengo que quedarme esperando hasta que por X razón una estación se re-asocie al AP?,
...
paramos el ataque, Â¿qué creéis que pasaría?. Fijaros que en la mayoría de las veces, los dispositivos tienden
...
Te preguntarás, Â¿y qué consigo con eso?, buena pregunta. Nuestro objetivo como atacantes es hacer siempre que
de una u otra forma, los clientes de una red sean reasociados para capturar un Handshake. 

Â¿Qué crees que pasaría si en una red inyectamos 5.000 clientes?, exacto, por ahí van los tiros. Si una red
...
A efectos prácticos, por el momento esto no genera ningún inconveniente, Â¿cómo autenticamos por tanto ahora a
...
```bash
â”Œâ”€[âœ—]â”€[root@parrot]â”€[/home/s4vitar]
â””â”€â”€â•¼ #mdk3 wlan0mon a -a 20:34:FB:B1:C5:53 # Dirección MAC del AP hacklab
```
...
**CONSIDERACIÃ“N**: También he especificado la dirección MAC del AP objetivo en **ghex** (**64:D1:54:88:BA:3C**)
...
Esto son buenas noticias, pues no nos sale ningún tipo de error, Â¡hemos construido un paquete válido!.
...
generando montón de paquetes Beacon con información falsa. Â¿Qué conseguimos con esto?, pues bueno, uno de los
...
**ANOTACIÃ“N:** En la práctica, no es muy efectivo.
...
Â¿Qué mejor que validar la captura con otra herramienta?, con **pyrit**. Pyrit es una herramienta bestial para
...
Ahora es cuando recapitulamos, Â¿qué tipo de paquete es el que
...
**ANOTACIÃ“N**: En mi opinión, recomiendo hacer uso del siguiente filtrado para este tipo de casos, donde
...
hemos visto que **Pyrit** lo detecta sin problemas, Â¿por qué aircrack no?, la respuesta es sencilla. A la hora
...
Hasta ahora hemos estado parseando redes específicas, pero, Â¿no te has parado a pensar en que también
podríamos hacer esto?:

* airodump-ng wlan0mon -w Captura

Es decir, capturar todo el tráfico de todas las redes disponibles en el entorno en un fichero. Â¿Por qué íbamos
...
Â¡Qué belleza!, de bastante utilidad incluso para visualizar los paquetes **Probe Request**, contemplando las
...
Â¿Por qué queremos generar un archivo **HCCAP**?, porque luego a través de la herramienta **hccap2john**
...
Ahora bien, Â¿cómo filtro el canal que me interesa?, sencillo... a través de la siguiente operación:
...
Â¿Qué es lo cómodo de este método?, pues que por ejemplo yo ahora viendo que la red **MOVISTAR_1677** tiene el
...
(7.000/10.000 contraseñas por segundo), Â¿hay alguna forma de ir más rápido aún?, Â¿hay alguna forma de
...
Â¿No has pensado en que todos estos pasos se podrían omitir, si contásemos con un diccionario de claves ya
precomputadas?. Me explico, Â¿y si en vez de tener un diccionario de contraseñas en texto claro, tenemos un
...
Hoy las contraseñas ya no se guardan sin cifrar â€“o eso se espera. Cuando los usuarios de una plataforma fijan
...
â€œadivinarâ€ la secuencia correcta de caracteres que constituye la contraseña durante tanto tiempo como haga
falta.

Este método puede combinarse con los llamados â€œdiccionariosâ€ de contraseñas. En estos archivos, que circulan
...
sorprendió mucho la última vez, Â¿verdad?, sin embargo, vamos a hacer que tomen un papel más importante.
...
Â¿Intentamos ir algo más rápido?
...
más que contentos, Â¿verdad?, pero es que se puede ir aún más rápido todavía.
...
Hasta ahora hemos visto cómo obtener las credenciales de acceso a una red inalámbrica. Ahora bien, Â¿qué pasa
...
Â¿Por qué?, porque todo lo que estamos capturando es el tráfico externo que recopilamos en modo monitor:
...
Entonces bien, Â¿qué hacemos?, vamos a usar la cabeza por unos momentos. Â¿Qué es lo que hace que los paquetes
que capturemos estén encriptados y no podamos ver el tráfico privado?, la propia contraseña de la red, Â¿no?,
Â¿y qué pasa si la tenemos?, Â¿no se supone que deberíamos ser capaces entonces de desencriptar estos paquetes?,
correcto.
...
Â¡Perfecto!, está en hexadecimal, pasémoslo a un formato algo más legible y veamos si podemos sacar algún dato
en claro:
...
conecten a nuestro Fake AP, Â¿por qué sin autenticarse?, porque nuestro Fake AP es de protocolo **OPN** :)
...
Â¿Qué pasa si la red no cuenta con clientes?, Â¿se puede averiguar la contraseña?, la respuesta es sí, y no...
...
Sencillo, Â¿verdad?, pues ya está, así de fácil. En el fichero **/root/bettercap-wifi-handshakes.pcap** ahora
...
punto, estaríamos igual que con **Bettercap**, es decir, tenemos la captura, Â¿y ahora qué?, descubrámoslo en el siguiente punto.
...
**ANOTACIÃ“N**: Para saber a qué redes pertenecen estos Hashes, tan sólo tenemos que visualizar el valor
...
que es la única herramienta que utilizo de este OS. Â¿Qué nos permite hacer **WPSPinGenerator**?, veámoslo con un ejemplo práctico.
...
Â¿Lo bueno de esto?, que no importa cuantas veces cambies la contraseña... pues si el PIN sigue siendo el mismo
...
Â¿Qué hacemos en este caso cuando la red está oculta?, bueno, sabemos que a nivel de filtrado no vamos a tener
...
de-autenticación sobre una de las estaciones presentes. Â¿Pero qué es lo bueno de esto?, que ni nosotros
...
**IMPORTANTE: En este punto, no entraré tanto al detalle como en las redes de protocolo WPA. Â¿Por qué?, porque
